...
def _(mo):
    ...
    year_dropdown = mo.ui.dropdown(options=list(range(190, 2101)), label="Year", value=2024)
    return
...
@app.cell
def _(body, datetime_picker, dt, get_body, obs_time, pd):
    ref_zodiac = [
        {"name": "aries",
    ...
    ref_zodiac["lon_rad"] = get_body(body, time=obs_time, location=None)
...
@app.cell
def _(body_color_map, get_body, np, obs_time, pd, u):
    tmp_data = []
...
@app.cell
def _(alt, body_color_map, df_geocentric):
    color_scale_gc = alt.Scale(
    ...
    chart_gc = (
        ...
        .encode(
            ...
            x=alt.X("x_norm", scale=alt.Scale(domain=[-11, 11], clamp=True)),
            y=alt.Y("y_norm", scale=alt.Scale(domain=[-11, 11], clamp=True)),
            color=alt.Color(
                "body:N",
                scale=color_scale_gc,
...
@app.cell
def _(SphericalRepresentation, get_body_barycentric, obs_time, pd, u):
    ...
    return df_heliocentric, planets, planets_colors


@app.cell
def _(alt, df_heliocentric, mo, planets, planets_colors):
